...
**Menú visible:** Inicio, Acerca, Servicios, **Tecnología**, Contacto (etiquetas en `common.nav.*`, `NAV_PAGES` en `Header.tsx`). Otras secciones sin enlace en menú.

**Acerca (`about`):**

- Relato profesional dual: software + análisis de datos; enlace a [Sea Dragon S.p.A.](https://www.seadragon.cl) en experiencia.
- **CV:** enlace al PDF por idioma desde `src/lib/cvUrl.ts` (archivos en [devrfc83/devrfc83](https://github.com/devrfc83/devrfc83/tree/main/cv)).
- **Intereses:** lista en `about.interests.*` + entradas especiales (ajedrez, kickboxing, radioafición, café) con `Trans` en `Acerca.tsx`; orden **alfabético según el locale activo** (`localeCompare`).
- **Historia académica:** redacción **propria por idioma** (no traducción literal del título chileno); el título intermedio Analista Programador no se usa en el sitio (sí puede estar en el CV).

**Tecnología (`technology`):**

- Página en `src/components/pages/Tecnologia.tsx` + `src/components/technology/TechStack.tsx`.
- Stack por capas (UI → SO): datos en `src/lib/techStack.ts` (iconos Font Awesome, enlaces oficiales); etiquetas de capa en `technology.layers.*`.
- Copy introductorio en `technology.description` (no Lorem ipsum).

**En stand-by (Lorem ipsum):** productos y proyectos — no expandir sin pedido explícito.
...
- Placeholder Lorem ipsum solo en **productos** y **proyectos** (stand-by).
...
- [ ] Tras editar JSON de locale: validar sintaxis (p. ej. comas duplicadas) antes de `npm run dev`.
